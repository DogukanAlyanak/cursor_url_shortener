...
class UrlShortenerController extends Controller
{
    ...
    public function reports()
    {
        ...
        $topUrls = ShortUrl::where('visits', '>', 0)
            ->orderBy('visits', 'desc')
            ->limit(5)
            ->get();
        ...
        $generalStats = [
            ...
            'total_visits' => ShortUrl::sum('visits'),
            'avg_visits' => round(ShortUrl::where('visits', '>', 0)->avg('visits') ?? 0, 2),
            'urls_without_visits' => ShortUrl::where('visits', 0)->count(),
            'most_visited_today' => ShortUrl::whereDate('updated_at', today())
                ->orderBy('visits', 'desc')
                ->first(),
            'total_visits_today' => ShortUrl::whereDate('updated_at', today())
                ->sum('visits'),
        ];
